...
/**
 * ClaseName：PMerchApplyIdcard
 * Description：
 * Author：chocozhao
 * QQ: 1027313530
 * Createtime：2019/8/26 14:43
 * Modified By：
 * Fixtime：2019/8/26 14:43
 * FixDescription：
 **/

public class PMerchApplyIdcard extends XPresent<MerchApplyIdcardActivity> {
                ...
                .subscribe(new ApiSubcriber<GetUploadFileIdResult>() {
                    ...
                    public void onNext(GetUploadFileIdResult result) {
                        if (result.getCode() == ResultCode.SUCCESS.code()) {
                            ...
                            if (fileType.equals("25")) {
                                getV().setIdCardInfoZM(result.getData());
                            } else if (fileType.equals("26")) {
                                getV().setIdCardInfoFM(result.getData());
                            } else if (fileType.equals("23")) {
                                getV().setIdCardInfoAuth(result.getData());
                            }
                        } else {
