 ...
 * FixDescription：
 */
public class PProcessPlanFragment extends XPresent<ProcessPlanFragment> {
    ...
    public void GetProcessPlan(final int page, int pageNum, String beginTime, String endTime, String merchId, String type, String status, String acctNo) {
        {
            ...
            Api.getAPPService().getProcessPlan(merchId, type, status, acctNo, page, pageNum, beginTime, endTime, version)
                    ...
                    .subscribe(new ApiSubcriber<GetProcessPlanResults>() {
                        @Override
                        public void onNext(GetProcessPlanResults getProcessPlanResults) {
                            if (getProcessPlanResults.getCode() == ResultCode.SUCCESS.code()) {
                                getV().setAdapter(getProcessPlanResults, page);
                            } else {
                                getV().showErrorView(getProcessPlanResults.getMessage());
                            }
