...
import com.yzf.king.adapter.TransMyDeviceAdapter;
...
import com.yzf.king.present.PTransMyDevice;
...
/**
 * ClaseName：MyDeviceFragment
 * Description：
 * Author：chocozhao
 * QQ: 1027313530
 * Createtime：2019/8/22 15:42
 * Modified By：
 * Fixtime：2019/8/22 15:42
 * FixDescription：
 **/

public class MyDeviceFragment extends XFragment<PTransMyDevice> {
    ...
    private TransMyDeviceAdapter adapter;
    ...
    public static MyDeviceFragment newInstance(String type) {
    ...
    public void initData(Bundle savedInstanceState) {
        ...
        getP().getTransMyDevice(AppUser.getInstance().getMerchId(), 1, pageSize,beginTime,endTime);
    }
    ...
    public void initAdapter() {
        if (adapter == null) {
            adapter = new TransMyDeviceAdapter(context);
            adapter.setRecItemClick(new RecyclerItemCallback<GetMyTermResult.DataBean.DataListBean, TransMyDeviceAdapter.ViewHolder>() {
                @Override
                public void onItemClick(int position, final GetMyTermResult.DataBean.DataListBean item, int tag, TransMyDeviceAdapter.ViewHolder holder) {
        ...
        recyclerview.setOnRefreshAndLoadMoreListener(new XRecyclerView.OnRefreshAndLoadMoreListener() {
            ...
            public void onLoadMore(int page) {
                getP().getTransMyDevice(AppUser.getInstance().getMerchId(),page,pageSize,beginTime,endTime);
            }
        });
    ...
    @Override
    public PTransMyDevice newP() {
        return new PTransMyDevice();
    }
    ...
    public void setAdapter(GetMyTermResult dataBean, int page) {
        if (dataBean != null) {
            shopCountTv.setText("我的设备总数："+dataBean.getData().getActiveCount()+"(台)");
        }
