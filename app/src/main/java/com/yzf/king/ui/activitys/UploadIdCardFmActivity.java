...
import com.yzf.king.present.PUploadIdCardFm;
 ...
 * FixDescription：
 */
public class UploadIdCardFmActivity extends XActivity<PUploadIdCardFm> {
    ...
    Toolbar toolbar;
    @BindView(R.id.ulidcard_fm_iv)
    ImageView ulidcardFmIv;
    @BindView(R.id.ulidcard_fm_tv)
    TextView ulidcardFmTv;
    @BindView(R.id.ulidcard_day_et)
    XEditText ulidcardDayEt;
    @BindView(R.id.ulidcard_agent_et)
    XEditText ulidcardAgentEt;
    ...
    public int getLayoutId() {
        return R.layout.activity_upload_idcardfm;
    }

    @Override
    public PUploadIdCardFm newP() {
        return new PUploadIdCardFm();
    }
    ...
    @OnClick({R.id.ulidcard_fm_iv, R.id.ulidcard_confirm_bt})
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.ulidcard_fm_iv:
                mCount = IDCARDBACK;
            ...
            case R.id.ulidcard_confirm_bt:
                ...
                if (flag) {
                    ...
                    String day = ulidcardDayEt.getTextEx();
                    String ageny = ulidcardAgentEt.getTextEx();
                    if (AppTools.isEmpty(day)) {
                        showToast("有效期空，请上传身份证反面照重试");
                        return;
                    }
                    if (AppTools.isEmpty(ageny)) {
                        showToast("发证机关为空，请上传身份证反面照重试");
                        return;
                    }
                    JumpActivity(UploadBankCardActivity.class, true, idNo, name);
                }
                break;
        }
    }

    public void setIdInfo(UploadPhotosResult.DataBean.IdBeanBean idBean) {
        ulidcardDayEt.setText(idBean.getInvalidDate().trim().replace(" ", ""));
        ulidcardAgentEt.setText(idBean.getAgency().trim().replace(" ", ""));
    }
        ...
        switch (file_type) {
            case "01":
                if (success.equals("success")) {
                    ILFactory.getLoader().loadImageSizekipMemoryCache(imgPath[0], ulidcardFmIv);
                    imgFlag[0] = "00";
                    ulidcardFmTv.setVisibility(View.GONE);
                } else if (success.equals("fail")) {
                    ILFactory.getLoader().loadImage( R.mipmap.loading_fail_img,ulidcardFmIv);
                    imgFlag[0] = "01";
                    ulidcardFmTv.setVisibility(View.VISIBLE);
                    ulidcardFmTv.setText(message);
                                ...
                                switch (mCount) {
                                    case IDCARDBACK:
                                        if (!context.isFinishing()) {
                                            ILFactory.getLoader().loadGif( "file:///android_asset/loading.gif",ulidcardFmIv);
                                        }
                                        imgPath[0] = file.getAbsolutePath();
//                                        getvDelegate().showLoading();
                                        getP().uploadPhoto(file, "01", AppUser.getInstance().getMerchId());
                                        break;
                                    default:
                                        break;
                                }
                ...
                .subscribe(new Action1<Boolean>() {
                    ...
                    public void call(Boolean granted) {
                        if (granted) {
                            ...
                            switch (mCount) {
                                case IDCARDBACK:
                                    CameraActivity.toCameraActivity(context, CameraActivity.TYPE_IDCARD_BACK);
                                    break;
                                default:
                                    break;
                            }
