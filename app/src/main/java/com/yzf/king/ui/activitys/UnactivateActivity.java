...
import com.yzf.king.adapter.UnactivateAdapter;
...
import com.yzf.king.present.PUnactivate;
...
public class UnactivateActivity extends XActivity<PUnactivate> {
    @BindView(R.id.title)
    TextView title;
    ...
    TextView myShopTv;
    private UnactivateAdapter adapter;
    ...
    @Override
    public PUnactivate newP() {
        return new PUnactivate();
    }
    ...
    public void initAdapter() {
        if (adapter == null) {
            adapter = new UnactivateAdapter(context);
            adapter.setRecItemClick(new RecyclerItemCallback<GetMyTermResult.DataBean.DataListBean, UnactivateAdapter.ViewHolder>() {
                @Override
                public void onItemClick(int position, final GetMyTermResult.DataBean.DataListBean item, int tag, UnactivateAdapter.ViewHolder holder) {
                    super.onItemClick(position, item, tag, holder);
                    switch (tag) {
                        case UnactivateAdapter.TAG_VIEW:
                            Router.newIntent(context)
                                    .to(ActivateDtlActivity.class)
                                    .putSerializable("dataListBean",item)
                                    .putString("unact","unact")
                                    .launch();
                            break;
                        default:
                    }
