...
import com.yzf.king.present.PUploadFace;
 ...
 * FixDescription：
 */
public class UploadFaceActivity extends XActivity<PUploadFace> {
    @BindView(R.id.title)
    TextView title;
    @BindView(R.id.toolbar)
    Toolbar toolbar;
    @BindView(R.id.ulface_iv)
    ImageView ulfaceIv;
    @BindView(R.id.ulface_tv)
    TextView ulfaceTv;
    @BindView(R.id.ulface_cap_iv)
    ImageView ulfaceCapIv;
    @BindView(R.id.ulface_confirm_bt)
    StateButton ulfaceConfirmBt;
    @BindView(R.id.ulface_jump_tv)
    TextView ulfaceJumpTv;
    ...
    public void initData(Bundle savedInstanceState) {
        initView();
        mMobile = AppUser.getInstance().getMerchId();
    }
    ...
    @Override
    public PUploadFace newP() {
        return new PUploadFace();
    }
    ...
    @OnClick({R.id.ulface_iv, R.id.ulface_cap_iv, R.id.ulface_confirm_bt,R.id.ulface_jump_tv})
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.ulface_iv:
            case R.id.ulface_cap_iv:
                mCount = FACE;
                takePhoto(mMobile + "_FACE");
                break;
            case R.id.ulface_confirm_bt:
                boolean flag = false;
                ...
                if (flag) {
                    ...
                    getP().faceMatch(AppUser.getInstance().getMerchId(), "0");
                }
                break;
            case R.id.ulface_jump_tv:
                JumpActivity(HomeFragment.class,true);
                break;
        }
    }
        ...
        switch (file_type) {
            case "03":
                if (success.equals("success")) {
                    ILFactory.getLoader().loadImageSizekipMemoryCache(imgPath[0], ulfaceIv);
                    imgFlag[0] = "00";
                    ulfaceTv.setVisibility(View.GONE);
                } else if (success.equals("fail")) {
                    ILFactory.getLoader().loadImage(R.mipmap.loading_fail_img, ulfaceIv);
                    imgFlag[0] = "01";
                    ulfaceTv.setVisibility(View.VISIBLE);
                    ulfaceTv.setText(message);
                                ...
                                switch (mCount) {
                                    case FACE:
                                        if (!context.isFinishing()) {
                                            ILFactory.getLoader().loadGif("file:///android_asset/loading.gif", ulfaceIv);
                                        }
                                        imgPath[0] = file.getAbsolutePath();
                                        getP().uploadPhoto(file, "03", AppUser.getInstance().getMerchId());
                                        break;
                                    default:
                                        break;
                                }
                ...
                .subscribe(new Action1<Boolean>() {
                    ...
                    public void call(Boolean granted) {
                        if (granted) {
                            ...
                            switch (mCount) {
                                case FACE:
                                    CameraActivity.toCameraActivity(context, CameraActivity.TYPE_PERSEN);
                                    break;
                                default:
                                    break;
                            }
