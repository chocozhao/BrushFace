...
import com.yzf.king.present.PTeamUnactivate;
...
public class TeamUnactivateActivity extends XActivity<PTeamUnactivate> {

    @BindView(R.id.title)
    TextView title;
    @BindView(R.id.toolbar)
    Toolbar toolbar;
    @BindView(R.id.unactivate_multiplestatusview)
    MultipleStatusView multiplestatusview;
    @BindView(R.id.unactivate_swiperefreshlayout)
    SwipeRefreshLayout swiperefreshlayout;
    @BindView(R.id.unactivate_recyclerview)
    ...
    public void initData(Bundle savedInstanceState) {
        initView();
        initAdapter();
        getP().getTeamUnactivate(AppUser.getInstance().getMerchId(),1,pageSize,beginTime,endTime);
    }

    @Override
    public int getLayoutId() {
        return R.layout.activity_team_unactivate;
    }

    @Override
    public PTeamUnactivate newP() {
        return new PTeamUnactivate();
    }
    ...
    public void initAdapter() {
        if (adapter == null) {
            adapter = new TeamActivateAdapter(context);
        ...
        recyclerview.setOnRefreshAndLoadMoreListener(new XRecyclerView.OnRefreshAndLoadMoreListener() {
            ...
            public void onLoadMore(int page) {
                getP().getTeamUnactivate(AppUser.getInstance().getMerchId(),page,pageSize,beginTime,endTime);
            }
        });
